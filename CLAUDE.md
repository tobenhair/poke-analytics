...
- `npm run cardmarket:ingest` (`scripts/cardmarket-ingest.mjs`) — the **production** counterpart of the spike: the scheduled job that writes. It is **DB-driven** — the tracked set is the Supabase `products` table (seed via Data Entry), not `cardmarket-map.json`. Each product's **`cardmarket_product_id`** (the Cardmarket `idProduct`, entered in the Data Entry "CM ID" column) **pins the catalogue match exactly**; the expansion id for the singles sum is derived from the catalogue. A product with no id falls back to name-matching. `cardmarket-map.json` is now just *overrides* (`nameHint` / `priceOverride`) plus the offline allowlist that `--dry-run` uses when there are no DB creds. It fetches the same bulk files, derives via the shared `scripts/cardmarket-lib.mjs` (one source of truth with the spike — `cardmarket-lib.test.mjs` pins it), and **upserts today's `snapshots` row into Supabase with the service-role key**. Set Value = `avg30` all-cards singles sum; Box Price = `trend`, **skipped when `products.price_locked`** (the thin-liquidity manual override, admin-set in Data Entry — Set Value still updates); each snapshot carries the `low_liquidity` flag. Products are not created here — a tracked product missing from Supabase is warned and skipped. `--dry-run` derives and prints without writing (no creds); `--backfill-ids` resolves each DB product that lacks a `cardmarket_product_id` and writes the confident match back onto it (one-time, so the DB becomes self-sufficient and later runs skip name-matching). Runs via `.github/workflows/cardmarket-ingest.yml` (daily cron + manual dispatch, with a `backfill_ids` input); needs `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` secrets for the real write. Schema: `products.cardmarket_product_id` + `products.price_locked` + `snapshots.low_liquidity` in `supabase/schema.sql`; operator setup in `SUPABASE.md`.
...
Static hosting — deploy by committing `index.html` and `pokemon_data.xlsx`. The intended monthly loop (see README): open the dashboard → enter the month's prices and set values in **Data Entry** → **Export updated .xlsx** → replace `pokemon_data.xlsx` in the repo and commit. That manual loop still works, but in Supabase mode a scheduled **Cardmarket ingestion job** can now write the daily snapshot automatically (`scripts/cardmarket-ingest.mjs` + `cardmarket-ingest.yml`; see `SUPABASE.md`), with a per-product price-lock for anything you'd rather set by hand.
